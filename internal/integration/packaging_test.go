...
func TestPackagingSmokeBuildsBinariesAndServiceReferencesDaemon(t *testing.T) {
	...
	for _, target := range []struct{ name, pkg string }{
		{"umbrel-dropbox-sync", "./cmd/umbrel-dropbox-sync"},
		{"umbrel-dropbox-syncd", "./cmd/umbrel-dropbox-syncd"},
	} {
	...
	service := readFile(t, filepath.Join(repo, "packaging/systemd/umbrel-dropbox-sync.service"))
	if !strings.Contains(service, "ExecStart=/usr/bin/umbrel-dropbox-syncd --config %h/.config/umbrel-dropbox-sync/config.json") {
		t.Fatalf("service ExecStart missing daemon config path:\n%s", service)
	}
	goreleaser := readFile(t, filepath.Join(repo, ".goreleaser.yaml"))
	for _, want := range []string{"binary: umbrel-dropbox-sync", "binary: umbrel-dropbox-syncd", "dst: /usr/lib/systemd/user/umbrel-dropbox-sync.service"} {
...
func TestInstallScriptInitializesDaemonConfig(t *testing.T) {
	...
	for _, want := range []string{
		"/usr/local/bin/umbrel-dropbox-sync",
		"/usr/local/bin/umbrel-dropbox-syncd",

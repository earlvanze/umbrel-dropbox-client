...
async function saveSettings(e){e.preventDefault();const root=$1('#setting-root').value.trim();if(!root){showToast('Root path is required','error');return}const sync=$1('#setting-syncpaths').value.split(',').map(s=>s.trim()).filter(Boolean);const excl=$1('#setting-exclude').value.split(',').map(s=>s.trim()).filter(Boolean);const cfg={root:root,remote_path:$1('#setting-remote').value.trim(),dry_run:$1('#setting-dryrun').value==='true',allow_live:$1('#setting-allowlive').value==='true',scan_interval_seconds:parseInt($1('#setting-interval').value)||300,watch:$1('#setting-watch').value==='true',sync_paths:sync,exclude_paths:excl,ignore_dirs:$1('#setting-ignore').value.trim()};try{const r=await fetch('/api/config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(cfg)});if(!r.ok){const txt=await r.text();throw new Error(txt.slice(0,200))}const data=await r.json();if(data&&data.restart_required){const fields=(data.restart_fields||[]).join(', ');if(confirm('Settings saved. The following changes require a daemon restart to take effect:\n\n  '+fields+'\n\nA full scan will run on the new configuration. Continue?')){await restartDaemon()}else{showToast('Restart skipped — new settings will apply on the next daemon restart','success')}}else{showToast('Settings saved','success')}}catch(err){showToast(err.message,'error')}}
async function restartDaemon(){showToast('Restarting daemon...','success');try{const r=await fetch('/api/restart',{method:'POST'});if(!r.ok){const txt=await r.text();throw new Error(txt.slice(0,200))}}catch(err){showToast('Restart request failed: '+err.message,'error');return}const deadline=Date.now()+30000;while(Date.now()<deadline){try{const r=await fetch('/api/status');if(r.ok){const s=await r.json();showToast('Daemon back online — root='+s.root,'success');loadDashboard();loadSettings();return}}catch(_){}await new Promise(r=>setTimeout(r,750))}showToast('Daemon did not come back within 30s; check the page','error')}
